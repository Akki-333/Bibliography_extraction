...
#: Where the batch page leaves its run.
_BATCH_RESULT = "pm_batch_result"
...
def _count(number: int, noun: str) -> str:
    """Return a count with its noun agreeing in number.
    ...
    Returns:
        Text such as ``"1 entry"`` or ``"117 entries"``.
    """
    plural = noun if number == 1 else f"{noun}s" if not noun.endswith("y") else f"{noun[:-1]}ies"
...
def _docx_bytes(rendered: list[FormattedReference], guide: StyleGuide) -> io.BytesIO:
    ...
    subtitle = doc.add_paragraph(
        f"Generated by PaperMint · {_count(len(rendered), 'reference')} "
        f"formatted in {guide.short_name}"
    )
...
def _batch_citations() -> list[Citation]:
    """Return every citation the batch page's last run produced.

    Returns:
        The citations across all files in the run, or an empty list when no
...
def _matches(citation: Citation, query: str) -> bool:
    """Test one citation against the narrowing box.
...
def _narrowed(citations: list[Citation], origin: str) -> list[Citation]:
    """Render the narrowing box and return the citations it leaves.

    A batch of five documents can carry three hundred references, and a reader
    who wants one of them should not have to scroll a formatted list to find
    it. What survives the box is what the page renders *and* what the download
    contains, so the file always matches the screen.

    Args:
        citations: Every citation the chosen source offers.
        origin: A short description of where they came from.

    Returns:
        The citations to format.
    """
    query = st.text_input(
        "Narrow this list",
        placeholder="Filter by title, author, year, venue, DOI or file name",
        key=_FILTER_KEY,
        label_visibility="collapsed",
    )
    kept = [c for c in citations if _matches(c, query.lower())] if query else list(citations)

    shown = _count(len(kept), "reference")
    if len(kept) == len(citations):
        st.caption(f"{shown}{f' from {origin}' if origin else ''}.")
    else:
        st.caption(
            f"{shown} of {len(citations)}{f' from {origin}' if origin else ''}. "
            "The downloads below contain exactly what is shown."
        )
    return kept
...
def render() -> None:
    ...
    from_analyzer = _analyzer_citations()
    from_batch = _batch_citations()

    options = [_FROM_PASTE]
    if from_batch:
        options.insert(0, _FROM_BATCH)
    if from_analyzer:
        options.insert(0, _FROM_ANALYZER)
    ...
    if source == _FROM_ANALYZER:
        ...
        citations = _narrowed(from_analyzer, getattr(result, "source_filename", ""))
    ...
    if citations:
        ...
        st.divider()
    elif source in (_FROM_ANALYZER, _FROM_BATCH):
        empty_state(
            "Nothing matches that filter",
            "Clear the box above to bring back every reference from this source.",
            icon_name="filter",
        )
    else:
        empty_state(
